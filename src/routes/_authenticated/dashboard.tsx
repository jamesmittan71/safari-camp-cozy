...
export const Route = createFileRoute("/_authenticated/dashboard")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Live operations dashboard with occupancy, arrivals, departures, cleaning and maintenance.",
      },
...
const OPEN_MAINTENANCE = new Set(["reported", "assigned", "in_progress", "waiting_for_parts", "inspection"]);
...
function Dashboard() {
  ...
  const { data: members = [] } = useList<TeamMemberRow>(
    "team_members",
    "*",
    "surname",
    { refetchInterval: REFRESH_MS },
  ...
  const { data: profiles = [] } = useList<ProfileRow>(
    "profiles",
    "*",
    "full_name",
    { refetchInterval: REFRESH_MS },
  ...
  const profileMap = useMemo(
    () => new Map(profiles.map((profile) => [profile.id, profile.full_name ?? profile.email ?? "Unknown user"])),
    [profiles],
  );
  ...
  const arrivalsToday = useMemo(
    () => allocations.filter((allocation) => allocation.arrival_date === day && allocation.status !== "cancelled"),
    [allocations, day],
  );
  const departuresToday = useMemo(
    () => allocations.filter((allocation) => allocation.departure_date === day && allocation.status !== "cancelled"),
    [allocations, day],
  );
  ...
  const doubleAllocationErrors = useMemo(() => {
    ...
    for (const allocation of activeAllocations) {
      ...
      const occupants = [allocation.bed_a, allocation.bed_b, allocation.bed_a_name, allocation.bed_b_name].filter(Boolean);
  ...
  const attentionItems = useMemo(
    () =>
      [
        ...
        },
        {
          ...
          count: members.filter((member) => member.employment_status === "active" && !allocatedStaffIds.has(member.id)).length,
          tone: "muted" as const,
    ...
    [allocatedStaffIds, doubleAllocationErrors, housekeepingCounts, maintenanceCounts, members, outOfServiceRooms.length],
  );
  ...
  const occupancyByCamp = useMemo(() => {
    ...
    const map = new Map<string, { id: string; camp: string; occupied: number; available: number; outOfService: number }>();
    ...
    for (const allocation of allocations) {
      if (allocation.created_at?.slice(0, 10) === day) {
        items.push({
          ...
          action: allocation.status === "checked_in" || allocation.arrival_date === day ? "Check In" : "Allocation",
  ...
  }, [allocationHistory, allocations, day, housekeepingHistory, profileMap, reportMap, reports, maintenanceHistory, taskMap]);
  ...
  const cleaningQueue = useMemo(
    () =>
      [...tasks]
        ...
        .sort((a, b) => {
          const overdueDiff = Number(isHousekeepingOverdue(b, day)) - Number(isHousekeepingOverdue(a, day));
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-5">
        ...
        <KpiCard icon={Users} label="Occupied Beds" value={occupiedBeds} tone="text-status-occupied" />
        <KpiCard icon={BedDouble} label="Available Beds" value={availableBeds} tone="text-status-available" />
        <KpiCard icon={ClipboardList} label="Occupancy %" value={`${occupancyPercent}%`} />
        <KpiCard icon={AlertTriangle} label="Dirty Tents" value={housekeepingCounts.dirty} tone="text-status-cleaning" />
        <KpiCard icon={Sparkles} label="Cleaning In Progress" value={housekeepingCounts.cleaning} tone="text-status-occupied" />
        <KpiCard icon={Eye} label="Inspection Required" value={housekeepingCounts.inspection} tone="text-status-maintenance" />
        <KpiCard icon={Wrench} label="Open Maintenance" value={maintenanceCounts.open} />
        <KpiCard icon={AlertTriangle} label="Critical Maintenance" value={maintenanceCounts.critical} tone="text-destructive" />
        <KpiCard icon={CalendarCheck} label="Today's Check-ins" value={arrivalsToday.length} tone="text-status-available" />
        <KpiCard icon={CalendarX} label="Today's Check-outs" value={departuresToday.length} tone="text-status-cleaning" />
      ...
      <section className="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
        <Card className="shadow-lodge">
          ...
          <CardContent className="grid gap-3 sm:grid-cols-2">
            {attentionItems.map((item) => (
              <AlertCard key={item.key} title={item.title} count={item.count} tone={item.tone} detail={item.detail} />
            ))}
          </CardContent>
        </Card>
        ...
        <Card className="shadow-lodge">
          ...
          <CardContent>
            {activities.length === 0 ? (
              <p className="text-sm text-muted-foreground">No operational activity recorded yet today.</p>
            ) : (
              <ul className="space-y-3">
                {activities.slice(0, 12).map((activity) => (
                  <li key={activity.id} className="flex items-start gap-3 border-b border-border pb-3 last:border-0 last:pb-0">
      ...
      <section className="grid gap-6 xl:grid-cols-2">
        <QueuePanel title="Arrivals Today" empty="No arrivals scheduled today.">
          {arrivalsToday.slice(0, 6).map((allocation) => (
            <QueueItemCard
              ...
              badge={<StatusBadge value={allocation.status === "checked_in" ? "occupied" : allocation.status} />}
              ...
              actions={
                <>
                  ...
                  <Button size="sm" onClick={() => completeArrival(allocation)} disabled={!canOperate || allocation.status === "checked_in"}>
                    Complete Task
                  </Button>
                </>
              }
            ...
            <QueueItemCard
              ...
              actions={
                <>
                  ...
                  <Button size="sm" onClick={() => completeDeparture(allocation)} disabled={!canOperate || allocation.status === "checked_out"}>
                    Complete Task
                  </Button>
                </>
              }
        ...
        <QueuePanel title="Cleaning Today" empty="No cleaning work is currently queued.">
          {cleaningQueue.slice(0, 6).map((task) => (
            <QueueItemCard
              ...
              meta={isHousekeepingOverdue(task, day) ? "Overdue cleaning" : `Assigned ${formatDate(task.date_assigned ?? task.created_at)}`}
              actions={
                <>
                  <Button asChild size="sm" variant="outline">
                    ...
                    onClick={() =>
                      setDetail({
                        ...
                        rows: [
                          ...
                          { label: "Assigned Cleaner", value: task.team_members ? `${task.team_members.name} ${task.team_members.surname}` : "—" },
                  ...
                  <Button size="sm" onClick={() => completeCleaning(task)} disabled={!canOperate || !HK_LABELS[task.status]}>
                    Complete Task
                  </Button>
                </>
              }
        ...
        <QueuePanel title="Maintenance Today" empty="No open maintenance work requires action right now.">
          {maintenanceQueue.slice(0, 6).map((report) => (
            <QueueItemCard
              ...
              meta={isMaintenanceOverdue(report, day) ? "Overdue maintenance" : `Target ${formatDate(report.target_date)}`}
              actions={
                <>
                  <Button asChild size="sm" variant="outline">
                    ...
                    onClick={() =>
                      setDetail({
                        ...
                        rows: [
                          ...
                          { label: "Technician", value: report.technician ? `${report.technician.name} ${report.technician.surname}` : "—" },
                  ...
                  <Button size="sm" onClick={() => completeMaintenance(report)} disabled={!canOperate || !MAINT_NEXT_LABEL[report.status]}>
                    Complete Task
                  </Button>
                </>
              }
      ...
      <section className="grid gap-6 xl:grid-cols-[1.15fr_0.85fr]">
                ...
                <div className="flex items-center justify-between gap-3">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      Occupied {camp.occupied} · Available {camp.available} · Out Of Service {camp.outOfService}
                    </p>
                  </div>
                ...
                <div className="h-3 overflow-hidden rounded-full bg-muted">
                  <div className="flex h-full">
                    ...
                    <div
                      ...
                      style={{ width: `${camp.occupied + camp.available + camp.outOfService ? Math.round((camp.outOfService / (camp.occupied + camp.available + camp.outOfService)) * 100) : 0}%` }}
                    />
                  </div>
                </div>
              </div>
        ...
        <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-1">
          <Card className="shadow-lodge">
            ...
            <CardContent className="grid gap-3 sm:grid-cols-2">
              <SummaryStat label="Dirty" value={housekeepingCounts.dirty} tone="text-status-cleaning" />
              <SummaryStat label="Cleaning" value={housekeepingCounts.cleaning} tone="text-status-occupied" />
              <SummaryStat label="Inspection" value={housekeepingCounts.inspection} tone="text-status-maintenance" />
              <SummaryStat label="Clean" value={housekeepingCounts.clean} tone="text-status-available" />
              <SummaryStat label="Completed Today" value={housekeepingCounts.completedToday} tone="text-status-available" />
            </CardContent>
          </Card>
          ...
          <Card className="shadow-lodge">
            ...
            <CardContent className="grid gap-3 sm:grid-cols-2">
              ...
              <SummaryStat label="Critical Jobs" value={maintenanceCounts.critical} tone="text-destructive" />
              <SummaryStat label="Overdue Jobs" value={maintenanceCounts.overdue} tone="text-destructive" />
              <SummaryStat label="Completed Today" value={maintenanceCounts.completedToday} tone="text-status-available" />
            </CardContent>
          </Card>
      ...
      <Dialog open={!!detail} onOpenChange={(open) => !open && setDetail(null)}>
        <DialogContent className="sm:max-w-lg">
          ...
          <dl className="space-y-3 text-sm">
            {detail?.rows.map((row) => (
              <div key={row.label} className="grid gap-1 border-b border-border pb-3 last:border-0 last:pb-0 sm:grid-cols-[140px_1fr]">
...
function QueuePanel({ title, empty, children }: { title: string; empty: string; children: ReactNode[] | ReactNode }) {
...
function SummaryStat({ label, value, tone = "text-foreground" }: { label: string; value: number; tone?: string }) {
  return (
    <div className="rounded-xl border border-border p-4">
      <p className="text-xs tracking-wide uppercase text-muted-foreground">{label}</p>
